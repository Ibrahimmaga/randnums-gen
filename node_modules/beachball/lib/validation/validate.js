...
function validate(options, validateOptionsOverride) {
    var _a;
    const validateOptions = Object.assign({}, defaultValidationOptions, validateOptionsOverride || {});
    // Validation Steps
    ...
    const untracked = workspace_tools_1.getUntrackedChanges(options.path);
    ...
    if (options.dependentChangeType && !isValidChangeType_1.isValidChangeType(options.dependentChangeType)) {
    ...
    if (validateOptions.allowFetching) {
        isChangeNeeded = isChangeFileNeeded_1.isChangeFileNeeded(options);
        ...
        if (options.disallowDeletedChangeFiles && areChangeFilesDeleted_1.areChangeFilesDeleted(options)) {
    ...
    if (!isChangeNeeded) {
        ...
        if (!validatePackageDependencies_1.validatePackageDependencies(bumpInfo)) {
