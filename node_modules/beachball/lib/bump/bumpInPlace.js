...
function bumpInPlace(bumpInfo, options) {
    ...
    Object.keys(changes).forEach(pkgName => {
        updateRelatedChangeType_1.updateRelatedChangeType(pkgName, changes[pkgName], bumpInfo, dependentChangeInfos, bumpDeps);
    });
    ...
    Object.keys(packageChangeTypes).forEach(pkgName => {
        bumpPackageInfoVersion_1.bumpPackageInfoVersion(pkgName, bumpInfo, options);
    });
    ...
    const dependentModifiedPackages = setDependentVersions_1.setDependentVersions(packageInfos, scopedPackages);
